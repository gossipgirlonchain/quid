...
- [x] Finish `casper.ts`: confirmed against the deployed contract (`byPackageHash` calls), real advance ids parsed from the `AdvanceIssued` CES event - verified live: the 2nd demo run issued, read **id 1** from the event, and repaid it ([issue](https://testnet.cspr.live/transaction/2512cb6b34cdd54e0d441b7f177839cc8642d6ba3dc3d32d1deb735967b3047c) → [repay](https://testnet.cspr.live/transaction/c00d1366334f2302001921b71ce6228ef9e5f89e2a4b5299759f6626af9af3f0)). `reputationOf` dictionary read still unverified (needs the dict UREF; the `AdvanceRepaid` event shows reputation on cspr.live meanwhile).
- [ ] Run `npm run dev` (no longer SIM) and confirm it issues a **real** advance on Testnet, then repays. **Everything is staged:** paste Plaid Sandbox `PLAID_CLIENT_ID`+`PLAID_SECRET` into agent/.env, run `npm run plaid:sandbox` (bootstraps the watched bank item, writes `PLAID_ACCESS_TOKEN`), optionally `DEMO_EXTRA_OUTFLOW_USD=300` to stage the shortfall, then `npm run dev`. Demo borrower wallet + $5k standing allowance already on-chain ([approve tx](https://testnet.cspr.live/transaction/d85b2f53681fa4d729c7f360260f8ddbd1b4f83f5f4c582fac641a131c5c2a0a)).
...
- [x] Seed the pool with Testnet stablecoin via `fund_pool` (team-seeded for MVP). Seeded $10,000 dUSDC on 2026-06-12: [approve](https://testnet.cspr.live/transaction/8d7e070bbb28c163907a2505b068e2636dd1f83b86bf1088863bb1e3a5e8e9c6) → [fund_pool](https://testnet.cspr.live/transaction/4de71a843a5a80b46fe1385310dab52534cc3fdcb6a19310880b8d6d7625d1c3) (`npm run seed` re-runs this).
...
- **Time advances to settlement (pairs with the off-ramp).** Cash-out settles in ~1-2 business days, so trigger on `initiate_by = businessDaysBefore(bill_date, CASHOUT_LEAD_DAYS + SAFETY_BUFFER_DAYS)` (defaults 2 + 1), not on the shortfall date, so funds land before the bill debits. Add the timing gate in `agent/src/decision.ts` (date dimension alongside the amount gate) and extend the projection horizon so the agent sees the bill with lead time. Use business-day math (skip weekends/bank holidays) and fold in the ACH daily cutoff. Handle late detection (today past `initiate_by`): notify-and-offer, a faster rail (same-day ACH / RTP), or keep the advance in the Quid wallet. Only matters once the real off-ramp replaces the simulated payout.
